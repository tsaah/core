...
{
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "server.loading, > Loading C++ scripts");
...
void AchievementScriptMgr::LoadDatabase()
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "server.loading, >> Loaded %u C++ scripts in %u ms", GetScriptCount(), WorldTimer::getMSTimeDiffToNow(oldMSTime));
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "server.loading, ");
}
    ...
    for (auto const& scriptName : _scriptNamesStore)
    {
        if (uint32 sid = getScriptId(scriptName.c_str()))
        {
            if (
                ...
                {
                    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "sql.sql, Script named '%s' is assigned in the database, but has no code!", scriptName.c_str());
                }
        }
    }
}
