...
public:
    ...
    bool OnCheck(Player* source, Unit* target, uint32 /*criteria_id*/) override
    {
        ...
        {
            BattleGround* bg = source->GetBattleGround();
            return bg && bg->GetTypeID() == BATTLEGROUND_WS && reinterpret_cast<BattleGroundWS*>(bg)->GetFlagState(player->GetTeam()) == BG_WS_FLAG_STATE_ON_BASE;
        }
        return false;
    }
};
...
// achievement_tilted (Argent Tournament) and achievement_not_even_a_scratch (Strand of the
// Ancients) referenced BC/Wrath-only content that doesn't exist in Vanilla, and were already
// permanently broken (hardcoded to always return false) even in the AzerothCore source this was
// ported from having been disabled here — removed rather than restored.
