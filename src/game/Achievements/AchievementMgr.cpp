...
// One field shorter than the tables' full column counts: the trailing `patch` column is consumed
// by SQLStorage::LoadProgressive's WHERE-clause filtering (see World.cpp), not copied into the struct.
...
void AchievementMgr::LoadFromDB(ObjectGuid guid, SqlQueryHolder* holder) {
    const auto achievementResult = holder->TakeResult(PLAYER_LOGIN_QUERY_LOADACHIEVEMENTS);
    const auto criteriaResult = holder->TakeResult(PLAYER_LOGIN_QUERY_LOAD_CRITERIA_PROGRESS);
...
void AchievementMgr::SendAchievementEarned(AchievementEntry const* achievement) const
{
    ...
    // Realm-first announcement. There is no WotLK-style SMSG_SERVER_FIRST_ACHIEVEMENT opcode (or
    // equivalent client UI) in the 1.12 protocol, so this is a plain realm-wide system message
    // rather than a structured packet - detection/bookkeeping (IsRealmCompleted/SetRealmCompleted)
    // already runs correctly before this point; this only restores the missing announcement.
    if (achievement->flags & (ACHIEVEMENT_FLAG_REALM_FIRST_KILL | ACHIEVEMENT_FLAG_REALM_FIRST_REACH)) {
        // If guild exists - announce its name to the server
        // If guild does not exist - announce the player's name
        if (achievement->flags & ACHIEVEMENT_FLAG_REALM_FIRST_KILL && guild) {
            std::string text = "<" + guild->GetName() + "> is the first to complete the achievement [" + achievement->name[0] + "]!";
            sWorld.SendWorldText(LANG_SYSTEMMESSAGE, text.c_str());
        } else {
            std::string text = std::string(GetPlayer()->GetName()) + " is the first player to complete the achievement [" + achievement->name[0] + "]!";
            sWorld.SendWorldText(LANG_SYSTEMMESSAGE, text.c_str());
        }
    }
    ...
    for (AchievementCriteriaEntryList::const_iterator i = achievementCriteriaList->begin(); i != achievementCriteriaList->end(); ++i)
    {
        ...
        switch (type)
        {
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_DEATH:
                {
                    ...
                    // Upstream (WotLK) excludes arena-personal-rating achievements from the
                    // generic death counter here (achievIdByArenaSlot). Vanilla has no arena
                    // system at all (no BattleGround::isArena()/GetArenaType()/ArenaTeam), so
                    // that exclusion doesn't apply and is intentionally omitted.
                    SetCriteriaProgress(achievementCriteria, 1, PROGRESS_ACCUMULATE);
                    break;
                }
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SKILLLINE_SPELLS:
                {
                    ...
                    for (PlayerSpellMap::const_iterator spellIter = GetPlayer()->GetSpellMap().begin(); spellIter != GetPlayer()->GetSpellMap().end(); ++spellIter)
                    {
                        ...
                        for (SkillLineAbilityMap::const_iterator skillIter = bounds.first; skillIter != bounds.second; ++skillIter)
                        {
                            ...
                            {
                                // do not add counter twice if by any chance skill is listed twice in dbc (eg. skill 777 and spell 22717)
                                ++spellCount;
                                break;
                            }
                        }
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_EARN_HONORABLE_KILL:
                SetCriteriaProgress(achievementCriteria, GetPlayer()->GetHonorMgr().GetTotalHK());
                break;
            case ACHIEVEMENT_CRITERIA_TYPE_HK_CLASS:
...
void AchievementMgr::SetCriteriaProgress(AchievementCriteriaEntry const* entry, uint32 changeValue, ProgressType ptype)
{
    ...
    if (achievement && achievement->name[0]) {
        ...
        do {
            ...
            // A missing category (e.g. filtered out by patch gating) must still terminate the
            // loop - categoryId was previously only reassigned on success, so a failed lookup
            // spun forever.
            if (!category)
                break;
            if (category->name[0])
...
void AchievementMgr::RemoveCriteriaProgress(const AchievementCriteriaEntry* entry)
{
    ...
    // Zero and mark dirty rather than erasing outright - SaveToDB() only looks at entries
    // still present in this map, so erasing here would never emit the DELETE for the row
    // already persisted in character_achievement_progress, leaving it to be silently
    // reloaded (resurrected) on the player's next login.
...
void AchievementMgr::CompletedAchievement(AchievementEntry const* achievement)
{
    ...
    // Titles are not restored here: VMaNGOS (Vanilla 1.12) has no title system at all (no
    // CharTitles.dbc loading, no sCharTitlesStore, no Player::SetTitle) - in-game titles are a
    // Burning Crusade+ client feature, not applicable to this codebase. reward->titleId is loaded
    // from SQL but intentionally left unused.
    ...
    if (reward->sender)
    {
        ...
        Item* item = reward->itemId ? Item::CreateItem(reward->itemId, 1, GetPlayer()->GetObjectGuid()) : nullptr;
        if (item)
            item->SaveToDB(); // save before send: prevents it being lost at next mail load if send fails

        // MailDraft's assignment operator is private (item cloning is too expensive to allow
        // implicit copies), so each branch below constructs and sends its own draft rather than
        // building one shared variable via conditional reassignment.
        ...
        else
        {
            ...
            int localeIdx = GetPlayer()->GetSession()->GetSessionDbLocaleIndex();
            if (localeIdx >= 0)
            {
                if (AchievementRewardLocale const* loc = sAchievementMgr->GetAchievementRewardLocale(achievement))
                {
                    if ((int32)loc->Subject.size() > localeIdx && !loc->Subject[localeIdx].empty())
                        subject = loc->Subject[localeIdx];
                    if ((int32)loc->Text.size() > localeIdx && !loc->Text[localeIdx].empty())
                        text = loc->Text[localeIdx];
                }
            }
...
void AchievementGlobalMgr::sendDynamicDataUpdate(WorldSession* session, uint32 sinceTimestamp) const {
    const auto playerGuid = session->GetPlayer()->GetGUIDLow();

    // Lets the addon hide achievements/criteria that don't apply to this server's patch
    // (achievement_dbc/achievement_criteria_dbc is an unfiltered retail dump spanning many patches).
    ChatHandler(session).PSendSysMessage("ACHI;HELLO_SERVER_PATCH;%u", sWorld.GetWowPatch());

    std::unique_ptr<QueryResult> achievementResult(CharacterDatabase.PQuery(
        "SELECT `achievement`, `date` FROM `character_achievement` WHERE `guid` = '%u' AND `date` > '%u'",
        playerGuid, sinceTimestamp));
    if (achievementResult) {
        do {
            Field* fields = achievementResult->Fetch();
            uint32 achievementId = fields[0].GetUInt16();
            ...
            ChatHandler(session).PSendSysMessage("ACHI;HELLO_UPDATE;AC;%u;%u", achievementId, date);
        } while (achievementResult->NextRow());
    }

    std::unique_ptr<QueryResult> criteriaResult(CharacterDatabase.PQuery(
        "SELECT `criteria`, `counter`, `date` FROM `character_achievement_progress` WHERE `guid` = '%u' AND `date` > '%u'",
        playerGuid, sinceTimestamp));
    if (criteriaResult) {
        do {
            Field* fields = criteriaResult->Fetch();
            ...
            ChatHandler(session).PSendSysMessage("ACHI;HELLO_UPDATE;CR;%u;%u;%u", id, counter, date);
        } while (criteriaResult->NextRow());
    }
...
bool AchievementGlobalMgr::HandleAddonMessage(WorldSession* session, std::string const& rawMessage) const {
    ...
    std::string::size_type tabPos = rawMessage.find('\t');
    if (tabPos == std::string::npos)
        return false;

    if (rawMessage.compare(0, tabPos, "ACHI") != 0)
        return false;

    std::string const payload = rawMessage.substr(tabPos + 1);

    Tokens tokens = StrSplit(payload, ";");
    if (tokens.size() < 2)
        return true; // real prefix matched, but payload malformed - still consume it

    std::string const& subType = tokens[0];
    if (subType == "HELLO") {
        if (tokens.size() != 3) {
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "AchievementGlobalMgr::HandleAddonMessage: malformed HELLO (%s)", payload.c_str());
            return true;
        }

        std::string const& addonVersion = tokens[1];
        uint32 lastDynamicDataTimestamp = GetUInt32ValueFromArray(tokens, 2);

        Player* player = session->GetPlayer();
        sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "AchievementGlobalMgr::HandleAddonMessage: HELLO from %s - addon version %s, last dynamic data timestamp %u",
...
void AchievementGlobalMgr::LoadAchievementCriteriaData()
{
    ...
    const auto result = WorldDatabase.Query("SELECT criteria_id, type, value1, value2, ScriptName FROM achievement_criteria_data");
...
void AchievementGlobalMgr::LoadCompletedAchievements()
{
    ...
    const auto result = CharacterDatabase.Query("SELECT achievement FROM character_achievement GROUP BY achievement");
...
void AchievementGlobalMgr::LoadRewards()
{
    ...
    //                                               0      1        2        3     4       5        6     7
    const auto result = WorldDatabase.Query("SELECT ID, TitleA, TitleH, ItemID, Sender, Subject, Body, MailTemplateID FROM achievement_reward");
...
void AchievementGlobalMgr::LoadRewardLocales()
{
    ...
    //                                               0   1       2        3
    const auto result = WorldDatabase.Query("SELECT ID, Locale, Subject, Text FROM achievement_reward_locale");
    ...
    do
    {
        ...
        int idx = sObjectMgr.GetOrNewIndexForLocale(locale);
        if (idx < 0)
            continue;
        ...
        if (!subject.empty())
        {
            if ((int32)data.Subject.size() <= idx)
                data.Subject.resize(idx + 1);
            data.Subject[idx] = subject;
        }
