...
void AchievementMgr::SetCriteriaProgress(AchievementCriteriaEntry const* entry, uint32 changeValue, ProgressType ptype)
{
    ...
    if (achievement && achievement->name[0]) {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "AchievementMgr::SetCriteriaProgress(%u) %s criteria(%s) for %s[%s], %u points (%s)", entry->ID, m_player->GetName(), entry->name[0], breadCrumbs.c_str(), achievement->name[0], achievement->points, achievement->description[0]);
    }
...
void AchievementMgr::CompletedAchievement(AchievementEntry const* achievement)
{
    ...
    if (m_player->IsGameMaster())
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "achievement Not available in GM mode.");
    ...
    if (achievement && achievement->name[0]) {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "AchievementMgr::CompletedAchievement(%u) %s %s[%s], %u points (%s)", achievement->ID, m_player->GetName(), breadCrumbs.c_str(), achievement->name[0], achievement->points, achievement->description[0]);
    }
...
void AchievementGlobalMgr::LoadAchievementCriteriaList()
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", ">> Loaded 0 achievement criteria.");
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", " ");
        return;
    }
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", ">> Loaded %u achievement criteria in %u ms", loaded, WorldTimer::getMSTimeDiffToNow(oldMSTime));
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", " ");
}
...
void AchievementGlobalMgr::LoadAchievementReferenceList()
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", ">> Loaded 0 achievement references.");
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", " ");
        return;
    }
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", ">> Loaded %u achievement references in %u ms", count, WorldTimer::getMSTimeDiffToNow(oldMSTime));
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", " ");
}
...
void AchievementGlobalMgr::LoadAchievementCriteriaData()
{
    ...
    if (!result)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", ">> Loaded 0 additional achievement criteria data. DB table `achievement_criteria_data` is empty.");
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", " ");
        return;
    }
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", ">> Loaded %u additional achievement criteria data in %u ms", count, WorldTimer::getMSTimeDiffToNow(oldMSTime));
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", " ");
}
...
void AchievementGlobalMgr::LoadCompletedAchievements()
{
    ...
    if (!result)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", ">> Loaded 0 completed achievements. DB table `character_achievement` is empty.");
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", " ");
        return;
    }
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", ">> Loaded %lu completed achievements in %u ms", (unsigned long)m_allCompletedAchievements.size(), WorldTimer::getMSTimeDiffToNow(oldMSTime));
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", " ");
}
...
void AchievementGlobalMgr::LoadRewards()
{
    ...
    if (!result)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", ">> Loaded 0 achievement rewards. DB table `achievement_reward` is empty.");
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", " ");
        return;
    }
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", ">> Loaded %u achievement rewards in %u ms", count, WorldTimer::getMSTimeDiffToNow(oldMSTime));
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", " ");
}
...
void AchievementGlobalMgr::LoadRewardLocales()
{
    ...
    if (!result)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", ">> Loaded 0 achievement reward locale strings.  DB table `achievement_reward_locale` is empty");
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", " ");
        return;
    }
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", ">> Loaded %lu Achievement Reward Locale strings in %u ms", (unsigned long)m_achievementRewardLocales.size(), WorldTimer::getMSTimeDiffToNow(oldMSTime));
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " server.loading", " ");
}
