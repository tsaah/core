...
bool Player::AddSpell(uint32 spellId, bool active, bool learning, bool dependent, bool disabled)
{
    ...
    // IsInWorld() matters here: AddSpell is called in tight bulk loops with the player not yet
    // in world (LearnDefaultSpells() at character creation, _LoadSpells() at login) - doing this
    // incremental bookkeeping there is both wasted (CheckAllAchievementCriteria() already covers
    // it in one cheap pass right after) and, for LEARN_SKILL_LINE/LEARN_SKILLLINE_SPELLS, actively
    // expensive (each rescans the player's entire currently-known spell list from scratch), which
    // compounds badly across a bulk grant of dozens of spells.
...
void Player::RewardQuest(Quest const* pQuest, uint32 reward, WorldObject* questEnder, bool announce)
{
    ...
    if (!pQuest->IsRepeatable()) {
        SetQuestStatus(questId, QUEST_STATUS_COMPLETE);
#ifdef USE_ACHIEVEMENTS
        UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_DAILY_QUEST_DAILY, questId);
#endif
    } else {
        SetQuestStatus(questId, QUEST_STATUS_NONE);
#ifdef USE_ACHIEVEMENTS
        UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_DAILY_QUEST, questId);
#endif
    }
...
#ifdef USE_ACHIEVEMENTS
uint32 Player::GetRewardedQuestCount() const
{
    uint32 count = 0;
    for (auto const& itr : mQuestStatus)
    ...
    if (!PvP)
    {
        ...
        if (!InBattleGround()) {
            if (!GetGroup() || IsAlive() || !GetCorpse()) {
                if (const Creature* target = pVictim->ToCreature()) {
...
void Player::HandleFall(MovementInfo const& movementInfo)
{
        ...
        !HasAuraType(SPELL_AURA_HOVER) && !HasAuraType(SPELL_AURA_FEATHER_FALL))
    {
        ...
        if (IsAlive() && final_damage < original_health)
            UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_FALL_WITHOUT_DYING, uint32(zDiff * 100));
...
void Player::RewardHonor(Unit const* pVictim, uint32 groupSize)
{
    ...
    if (pVictim->GetTypeId() == TYPEID_UNIT)
    {
        ...
        const Player* pVictim = cVictim->ToPlayer();
        if (pVictim) {
            ...
            // Recomputes from GetHonorMgr().GetTotalHK() (already-correct lifetime counter), so
            // this is safe to call here on every PvP kill instead of only via the .reset honor
            // GM command (its only other call site).
...
void Player::UpdateAchievementCriteria(AchievementCriteriaTypes type, uint32 miscValue1, uint32 miscValue2, Unit const* unit) {
